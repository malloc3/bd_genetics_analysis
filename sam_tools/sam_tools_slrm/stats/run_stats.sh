...
base_save_folder="./""$save_folder_name_prefix""stats""$todays_date"
...
for i in "${!sam_file_names[@]}"; do
    ...
    output_name=$base_save_folder"/""samtools-stats""$today_day""--""$only_file_name"".txt"  # should give me just the file name from the whole path
    ...
    #conda run -n $conda_environ /home/cmallory/Genetic_analysis/bd_genetics_analysis/sam_tools/sam_bin/bin/samtools stats -X $sam_file_path > $sam_results_save_file

    conda run -n $conda_environ samtools stats $sam_file_path > $sam_results_save_file
